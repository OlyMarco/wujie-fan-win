...
Self-drawn tray icon, live RPM/temperature readout, preset switching, a per-fan custom slider, and a `max` mode that rides CPU load between configurable bounds. No installer, no runtime, no telemetry — just C and a kernel I/O driver.
...
- **Systray UI** — self-drawn fan icon, right-click menu with live `FAN1`/`FAN2` RPM, CPU temp, current mode, and one-click presets.
...
- **Dynamic `max`** — target smoothly tracks CPU load between `max_min` and `max_max` (default 80–123), no GPU dependency.
- **Boot ramp** — `fanboot` waits `boot_delay_ms` after logon then applies `boot_preset` (default: 15s → silent).
...
```
src/              C sources (ec_fan, fantray, fanctl, fanboot, diag tools)
...
## Install (autostart on logon)

```powershell
PS> scripts\install-tasks.ps1
```

Registers `FanTray` and `FanBoot` scheduled tasks (logon trigger, highest privileges). Re-run after moving the project to update paths.
...
- **"EC init failed (need admin + WINIO driver)"** — run elevated; ensure the WinIo driver service is installed and test-signing is on.
